...
# \- \*\*Class imbalance handled twice\*\*: ISIC is dominated by benign nevi, so training uses both a weighted random sampler and a class-weighted loss. Model selection is on \*\*macro-AUC\*\*, not accuracy, which would otherwise reward a model that just predicts the majority class.
...
# \- The \*\*focus score is a crude entropy heuristic\*\*. It reliably flags genuinely off-target images, but it under-scores legitimate lesions that fill most of the frame, since attention naturally spreads in those cases. A size-normalized or blob-fraction metric would behave more intuitively — a planned improvement.
...
# \#    then sort into class folders:

# python prepare\_isic2019.py --csv data/ISIC\_2019\_Training\_GroundTruth.csv \\

# &#x20;   --images data/ISIC\_2019\_Training\_Input --out data/isic --per-class 1500

# 

# \# 2. Train (saves skinsight\_efficientnet\_b0.pt on best validation macro-AUC):

# python skinsight\_model.py
